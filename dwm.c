	...
	for (c = m->clients; c; c = c->next) {
     ...
     for (i = 0; i < LENGTH(tags); i++) {
        ...
        if (m->tagset[m->seltags] & 1 << i) {
            ...
            /* Empty tag (this branch is redundant but safe) */
            drw_setscheme(drw, scheme[SchemeNorm]);
        }
    	...
    	if ((w = m->ww - tw - x) > bh) {
    		if (m->sel) {
    			drw_setscheme(drw, scheme[m == selmon ? SchemeNorm : SchemeTag]);
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.col_border", col_border);
        XRDB_LOAD_COLOR("dwm.col_fade", col_fade);
        XRDB_LOAD_COLOR("dwm.col_tag", col_tag);
      }
    }
  }
...
setup(void)
{
    ...
    signal(SIGUSR1, sigxrdb);  // Add this line
